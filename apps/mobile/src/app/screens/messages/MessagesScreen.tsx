...
import { acitvitiesActions, selectDirectsOpenlistOrder, useAppDispatch } from '@mezon/store-mobile';
...
const MessagesScreen = () => {
	...
	const { pinned, unpinned } = useMemo(
		() => ({
			pinned: pinnedIds.filter((id) => dmGroupChatList?.includes(id)),
			unpinned: (dmGroupChatList ?? []).filter((id) => !pinnedIds.includes(id))
		}),
