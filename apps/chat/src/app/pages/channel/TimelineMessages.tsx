...
import type { BooleanToVoidFunction } from '@mezon/utils';
import {
	Direction_Mode,
...
function TimelineMessages({
	...
	useSyncEffect(() => {
		...
		return () => {
			...
			const scrollPosition = selectScrollPositionByChannelId(state, effectiveChannelId, 'timeline');
	...
	const handleScrollDownVisibilityChange = useCallback(
		(isVisible: boolean) => {
			...
			const showFAB = selectShowScrollDownButton(store.getState(), effectiveChannelId, 'timeline');
...
const ScrollDownButton = memo(
	({
		...
		const isVisible = useAppSelector((state) => selectShowScrollDownButton(state, effectiveChannelId, 'timeline'));
...
const TimelineMessageList: React.FC<TimelineMessageListProps> = memo(
	({
		messageIds,
		chatRef,
		...
		useSyncEffect(() => {
			const store = getStore();
			const state = store.getState();
			let scrollPosition = selectScrollPositionByChannelId(state, effectiveChannelId, 'timeline');
			if (!scrollPosition?.messageId) {
				if (lastMessageUnreadId && !hasScrolledToUnreadMap.get(`${effectiveChannelId}_timeline`) && messageIds?.length > 0) {
					scrollPosition = { messageId: lastMessageUnreadId };
					hasScrolledToUnreadMap.set(`${effectiveChannelId}_timeline`, true);
				}
			}
		...
		useLayoutEffectWithPrevDeps(
			([prevMessageIds, prevIsViewportNewest]) => {
				...
				requestForcedReflow(() => {
					...
					const store = getStore();
					const isAtBottom = !selectShowScrollDownButton(store.getState(), effectiveChannelId, 'timeline');
		...
		const groupedMessagesMap = useMemo(() => {
			...
			filteredMessageIds.forEach((messageId, index) => {
				...
				const isWithin1Hour = message?.create_time_seconds && timeDiff < TIME_COMBINE_1_HOUR;

				if (isSameUser && isWithin1Hour && currentGroupId) {
					groups.get(currentGroupId)?.push(message);
				} else {
					currentGroupId = messageId;
					groups.set(messageId, [message]);
				}
			});
...
const MemoizedTimelineMessages = memo(
	TimelineMessages,
	(prev, cur) => prev.channelId === cur.channelId && prev.avatarDM === cur.avatarDM && prev.channelLabel === cur.channelLabel
) as unknown as typeof TimelineMessages & {
	Skeleton: typeof TimelineMessages.Skeleton;
};
