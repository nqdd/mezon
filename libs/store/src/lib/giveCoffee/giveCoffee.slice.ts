...
export const updateGiveCoffee = createAsyncThunk(
	...
	async ({ channel_id, clan_id, message_ref_id, receiver_id, sender_id, token_count }: ApiGiveCoffeeEvent, thunkAPI) => {
		try {
			...
			const senderWalletAccount = await mezon.mmnClient.getAccountByUserId(sender_id);

			const response = await mezon.mmnClient.sendTransaction({
				sender: sender_id,
				recipient: receiver_id,
				amount: mezon.mmnClient.scaleAmountToDecimals(token_count ?? 0, senderWalletAccount.decimals),
...
export const sendToken = createAsyncThunk('token/sendToken', async (tokenEvent: ApiTokenSentEvent, thunkAPI) => {
	try {
		...
		const senderWalletAccount = await mezon.mmnClient.getAccountByUserId(tokenEvent.sender_id);

		const response = await mezon.mmnClient.sendTransaction({
			sender: tokenEvent.sender_id,
			recipient: tokenEvent.receiver_id,
			amount: mezon.mmnClient.scaleAmountToDecimals(tokenEvent.amount ?? 0, senderWalletAccount.decimals),
			nonce: senderWalletAccount.nonce + 1,
