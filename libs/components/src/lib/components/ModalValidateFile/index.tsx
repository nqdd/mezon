		...
		<ModalLayout onClose={onClose}>
			<div className="bg-red-500 rounded-lg relative p-3" data-e2e={generateE2eId('modal.validate_file')}>
				<div className="space-y-6 h-52 border-dashed border-2 flex text-center justify-center flex-col p-6">
					<Icons.FileAndFolder />
					<h3 className="text-white text-3xl font-semibold" data-e2e={generateE2eId('modal.validate_file.title')}>
						{title}
					</h3>
					<h4 className="text-white text-xl" data-e2e={generateE2eId('modal.validate_file.content')}>
						{content}
					</h4>
				</div>
			</div>
